...
with open(outfilename, 'w') as quickref:
    ...
    with open(manualxml, 'r') as manual:
        text = manual.read()
    ...
    for filename in files:
        ...
        elif synopsis := xmldoc.getElementsByTagName('synopsis'):
            # There can be more than 1 synopsis per file
            for synops in synopsis:
                tmp = synops.toxml()
                opcodename = (
                    tmp[tmp.find('<command>') + 9 : tmp.find('</command>')]
                    if XO
                    else ""
                )
        ...
        for j, thiscategory in enumerate(categories):
            if (category == thiscategory):
                ...
                match = True
        if not match:
            print(f"{str(filename)}---- WARNING! No Category Match!")

    for i in range(len(categories)):
        if (len(entries[i])==0):
            print(f"No entries for category: {categories[i]}...Skipping")
            continue
        ...
        for _ in range(len(entries[i])):
            newentry = entries[i].pop(0)
                    ...
                    # entry = entry.replace("&#160;", " ")
            quickref.write(f"<opcode><desc>{description}")
            quickref.write(f"{newentry[1]}</desc>")
            quickref.write(newentry[0] + "</opcode>\n")
            count += 1
        quickref.write("</category>\n")
        print(f"{str(count)} entries in category: {categories[i]}")
